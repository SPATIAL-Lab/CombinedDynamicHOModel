...
HOdriver = function (Angry_Bird) { # Table for a single B1 bird from AB model (i.e., Jim) 
...
# Assumes a single undifferentiated res pool but tracks two O isotope compositions: 1-that of carb+fat component of res pool, which is influenced by O isotope compositions of ingested food (f(d18Opcarb,d18Opfat)); and 2-that of prot component, which is additionally influenced by the O exchange of amino acids with gut water during digestion ( f(d18Opprot,d18Ogw))
...
state = c("O2"=O2, "D2res"=D2res, "O_18res"=O_18res, "O_18resprot"=O_18resprot, "H2"=H2, "D2bw"=D2bw, "O_18bw"=O_18bw)
...
parameters = list("BSs"=BSs, "Eat.nos"=Eat.nos, "DrinkYNs"=DrinkYNs, "Habitats"=Habitats, 
"HungerIncreaseNight"=HungerIncreaseNight, "HungerIncreaseDay"=HungerIncreaseDay)


out = deSolve::ode(y = state, times = times, func= HOmodel, parms = parameters, method = "rk4")
...
HOmodel = function(t, state, parameters) {
    with(as.list(c(state, parameters)), {
      ...
      # Depends on BS. In AB model hunger increases by 5 prey per hour during the day, and by 1 prey per hour at night, meaning that day-time FMR is 5 times greater than night-time FMR; because day-time metabolic rate reflects field metabolic rate and night-time metabolic rate resting metabolic rate, we assume that day-time metabolic rate equals FMR, and that night-time metabolic rate equals FMR / 5 
      FMR = if (BS == "D") {FMR} else {FMR / 5}
      
      # Variation in hunger increase rate (day and/or night) in AB model is coupled by variation in FMR (day and/or night) in phys/food model. Hungrier inds. eat more, and thus have a higher FMR; i.e., an ind. with, say, 10% higher hunger increase rate (day) has 10% higher FMR (day)  
      # In B2A, individual variation in hunger increase rates (day&night) - relative to the mean for the species - is coupled to variation in FMR (day&night)
      FMR = if (BS == "D") {FMR + FMR * ((( ((HungerIncreaseDay*100)/5)-100 ))/100)} else {FMR + FMR * ((( ((HungerIncreaseNight*100)/1)-100 ))/100)}   
      ...
      if (exp %in% c("E1_rl","E2A_rl","E2B_rl","E2C_rl","E1B1_rl")) { 
      ## Depend on habitat based on isoscape models 	
            	
      d18Od2Hp = if (round(Eat.no,digits=0) == 0) { # If Eat.no = 0, single d2H,d18O prey value is pulled but not used  
      	if (Habitat == 1) {
      		mvrnorm(n = 1, mu = c(mean(d18Op.MEAD)-avgd18Ooff_MR,mean(d2Hp.MEAD)-avgd2Hoff_MR), Sigma = cov(cbind(d18Op.MEAD,d2Hp.MEAD)))
      	} else {
      		if (Habitat ==2) {
      			mvrnorm(n = 1, mu = c(mean(d18Op.MEAD),mean(d2Hp.MEAD)), Sigma = cov(cbind(d18Op.MEAD,d2Hp.MEAD)))
      		} else {
      			mvrnorm(n = 1, mu = c(mean(d18Op.MEAD)+avgd18Ooff_SM,mean(d2Hp.MEAD)+avgd2Hoff_SM), Sigma = cov(cbind(d18Op.MEAD,d2Hp.MEAD)))
      		}
      	}
      } else { # If Eat.no is > 0, n d2H,d18O prey values are pulled - where n = Eat.no - and avg is used 
      	      	if (Habitat == 1) {
      		mvrnorm(n = round(Eat.no,digits=0), mu = c(mean(d18Op.MEAD)-avgd18Ooff_MR,mean(d2Hp.MEAD)-avgd2Hoff_MR), Sigma = cov(cbind(d18Op.MEAD,d2Hp.MEAD)))
      	} else {
      		if (Habitat ==2) {
      			mvrnorm(n = round(Eat.no,digits=0), mu = c(mean(d18Op.MEAD),mean(d2Hp.MEAD)), Sigma = cov(cbind(d18Op.MEAD,d2Hp.MEAD)))
      		} else {
      			mvrnorm(n = round(Eat.no,digits=0), mu = c(mean(d18Op.MEAD)+avgd18Ooff_SM,mean(d2Hp.MEAD)+avgd2Hoff_SM), Sigma = cov(cbind(d18Op.MEAD,d2Hp.MEAD)))
      		}
      	}
      }
      ...
      d2Hp = mean(d2Hp_each_p) 
      	...
      	if (exp %in% c("B1_rl","B2A_rl","B2B_rl","B2C_rl")) {  
      	## It is constant
      ...
      Ffw_hour = ( (Ms/MwH2O) * (Pw/(1-Pw)) * O2_per_prey*Eat.no )
                  
      # Flux of drinking water in [mol H2O d^-1] 
      # Depends on whether you are drinking at current time step
      # Determined so that avg dyn Fdw value approx. matches SS Fdw value, assuming that birds represented here are close to balance; also rescaled so that modeled TWF is consistent with measured TWF data for birds       
      Fdw = if(DrinkYN == 1) {   (  ((round(ssBird$Fdw,digits=2) *8) *1.410256)   *25)/100   } else {0}
      ...
      # Depends on H2
      # First condition allows for increased liquid water excretion to restore body water pool to minimum threshold size with e-folding time of 0.08 d (2 h)
      # Second condition imposes minimum Flw flux that depletes body water pool at a rate of 10% of threshold size per 7 d  
      ...
      # % enrichment; maximum possible enrichment * % enrichment = expressed enrichment; % enrichment: right-skewed in riparian (most samples have little enrichment, a few have lots); symmetrical in meadow, left-skewed in slope
      evaps = if (round(Eat.no,digits=0) == 0) { if (Habitat == 1) {rbeta(1,2,4)} else {if (Habitat == 2) {rbeta(1,2,2)} else {rbeta(1,4,2)}} } else { if (Habitat == 1) {rbeta(round(Eat.no,digits=0),2,4)} else {if (Habitat == 2) {rbeta(round(Eat.no,digits=0),2,2)} else {rbeta(round(Eat.no,digits=0),4,2)}} }
      ...
      if (Habitat == 1) {
      	...
      	d18Od2Hew = mvrnorm(n = 1, mu = c(mean(d18Oew.RIP),mean(d2Hew.RIP)), Sigma = cov(cbind(d18Oew.RIP,d2Hew.RIP)))
      	
      } else {if (Habitat == 2) {
      		
      	d18Od2Hew.RIP = mvrnorm(n = 1, mu = c(mean(d18Oew.RIP),mean(d2Hew.RIP)), Sigma = cov(cbind(d18Oew.RIP,d2Hew.RIP)))
      ...
      # O isotope composition of metabolic O; assumes that all macronutrients in the res pool are metabolized
      ...
      # O isotope composition of gut water; food water d18O influences keratin d18O during digestion(in the gut); thus, ultimately both 1- ingested protein d18O, and 2-gut water d18O influence keratin d18O through res pool d18O 
      ...
      dO_18res = (O2in_hour*(Pcarb*ROpcarb + Pfat*ROpfat)) - (O2out_hour*ROres*(Pcarb + Pfat)) 
      dO_18resprot = (O2in_hour*(PfO*ROpprot + (1-PfO)*alphaOc_w*ROgw)*Pprot) - (O2out_hour*ROresprot*Pprot) 
      
      dH2 = FfH_hour + Ffw_hour + Fdw_hour - (Fvw_hour + FwasteH_hour + Flw_hour)
      dD2bw = FfH_hour*RHf + Ffw_hour*RHfw + Fdw_hour*RHdw - (Fvw_hour*RHbw*alphaHbw_vw + FwasteH_hour*RHbw + Flw_hour*RHbw)	
      dO_18bw = FfO_hour*ROf + Ffw_hour*ROfw + Fdw_hour*ROdw + FO2_hour*ROo2*alphaOatm_abs - (Fvw_hour*RObw*alphaObw_vw + FwasteO_hour*RObw + Flw_hour*RObw + FCO2_hour*RObw*alphaObw_CO2)
      		...
      		"d2Hins"=d2Hp, "d2Hinsprot"=d2Hpprot, "d2Hinscarb"=d2Hpcarb, "d2Hinsfat"=d2Hpfat,
      		"d18Oins"=d18Op, "d18Oinscarb"=d18Opcarb, "d18Oinsfat"=d18Opfat, "d18Oinsprot"=d18Opprot, "d18Ogw"=d18Ogw,
      		"d2Hres"=R.to.delta(D2res/O2,RstandardH), "d18Ores"=R.to.delta(O_18res/(Pcarb+Pfat)/O2,RstandardO), "d18Oresprot"=R.to.delta(O_18resprot/Pprot/O2,RstandardO)), 
      
      		c("DrinkYN"=DrinkYN,
      		"FMR"=FMR_hour,
      		"FfH"=FfH_hour, "Ffw"=Ffw_hour, "Fdw"=Fdw_hour, "Fvw"=Fvw_hour, "FwasteH"=FwasteH_hour, "Flw"=Flw_hour, "FfO"=FfO_hour, "FO2"=FO2_hour, "FwasteO"=FwasteO_hour, "FCO2"=FCO2_hour, 
      		...
      		"d2Hbw"=R.to.delta(D2bw/H2,RstandardH), "d18Obw"=R.to.delta(O_18bw/H2,RstandardO))
